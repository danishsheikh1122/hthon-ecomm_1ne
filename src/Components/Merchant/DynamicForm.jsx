...
import { addProductToDB, checkSKU, getProductConfig } from "../../Api/api";
...
export default function DynamicForm(props) {
  ...
  const onSubmit = async (data) => {
    ...
    props.addProduct.mutate(formData,{
      onSuccess : (data)=>{
        reset()
      }
    })
  ...
  if (watch("hasVariation") === "No") {
    ...
    if(getValues("selectedVariations").length > 0) setValue("selectedVariations", [])
  }
  ...
  return (
    ...
    <>
     {form && <form onSubmit={handleSubmit(onSubmit)} className="form-container">
                    ...
                    {fields.map((item, index) => {
                      return (
                        <tr>
                          ...
                          {watch(`variants.${index}.Images`)?.length < 1 && (
                            <td>
  ...
  if (Array.isArray(set)) {
    return (
      <div className={`${type}-container`}>
        ...
        {formEnd && (
          <button type="submit" className="form-submit" disabled={props.addProduct.isPending}>
            Submit
          </button>
        )}
...
function generateField(
  ...
  const validation = field.validation ? field.validation : ()=> {return () => true}
          ...
          {watch(field.dependsOn.field) === field.dependsOn.value && (
            <div className="input-wrapper">
              ...
              <input
                ...
                placeholder={field.placeholder}
                step={field.step || ""}
                {...register(field.name ,{ ...field.schema , validate : validation(getValues) })}
              ></input>
    ...
    return (
      <div className="input-wrapper">
        ...
        <input
          ...
          style={style}
          step={field.step || ""}
          {...register(field.name ,{ ...field.schema , validate : validation(getValues,checkSKU) })}
        ></input>
        <p className="input-error">{get(errors , field.name)?.message}</p>
      </div>
    );
